    ...
    def require_active(self, skill_id: str) -> SkillAdmission:
        if not self.promotable:
            raise SkillAdmissionError(
                f"{skill_id} cannot be ACTIVE before all evaluations PASS"
            )
        return SkillAdmission(skill_id, "1.0.0", "ACTIVE")


@dataclass(frozen=True)
class SkillSourceReconciliation:
    sources: dict[str, Path]
    superseded_sources: tuple[Path, ...]
...
class SkillRegistry:
    ...
    def from_document(cls, document: dict[str, Any]) -> SkillRegistry:
        ...
        for category in _CATEGORIES:
            ...
            for value in values:
                if not isinstance(value, str) or not value.startswith("factory-"):
                    raise SkillAdmissionError(
                        f"registry.{category} entries must be canonical factory-* ids"
                    )
        ...
        for skill_id in proposed:
            if not isinstance(skill_id, str) or not skill_id.startswith("factory-"):
                raise SkillAdmissionError(
                    "proposed Skill ids must be canonical factory-* ids"
                )
        ...
        for source_id, target_id in aliases_raw.items():
            ...
            if not target_id.startswith("factory-") or target_id not in registered:
                raise SkillAdmissionError(
                    f"Skill alias target {target_id} is not registered canonical identity"
                )
            aliases[source_id] = target_id

        superseded_raw = raw.get("superseded_skill_concepts", {})
        if not isinstance(superseded_raw, dict):
            raise SkillAdmissionError("superseded_skill_concepts must be a mapping")
        superseded = frozenset(str(value) for value in superseded_raw)
        ...
        for agent_id, policy in consumers_raw.items():
            ...
            for key in ("required", "task_optional"):
                ...
                if not isinstance(values, list):
                    raise SkillAdmissionError(
                        f"consumer {agent_id}.{key} must be a list"
                    )
                result: list[str] = []
                for skill_id in values:
                    if not isinstance(skill_id, str):
                        raise SkillAdmissionError(
                            f"consumer {agent_id}.{key} has invalid id"
                        )
                    canonical = aliases.get(skill_id, skill_id)
                    if canonical not in registered:
                        raise SkillAdmissionError(
                            f"consumer {agent_id} references Skill {canonical} not registered"
                        )
    ...
    def reconcile_sources(self, root: Path) -> SkillSourceReconciliation:
        sources: dict[str, Path] = {}
        superseded: list[Path] = []
        orphans: list[str] = []
        duplicates: list[str] = []
        for path in sorted(root.rglob("SKILL.md")):
            name, _, _ = _parse_skill(path)
            if name in self._superseded:
        ...
        missing = sorted(self._registered - set(sources))
        if orphans or duplicates or missing:
            raise SkillAdmissionError(
                f"Skill source reconciliation failed: orphan={sorted(orphans)} "
                f"duplicate={sorted(duplicates)} missing={missing}"
            )
        return SkillSourceReconciliation(
            sources=sources,
        ...
        *,
        ...
        for raw_skill in task_approved:
            try:
                ...
                raise SkillAdmissionError(
                    f"Skill {raw_skill} is not authorized for {agent_id}"
                ) from exc
            if canonical not in allowed:
                raise SkillAdmissionError(
                    f"Skill {canonical} is not authorized for {agent_id}"
                )
...
def _parse_skill(source: Path) -> tuple[str, dict[str, Any], str]:
    ...
    if not isinstance(name, str) or not name:
        raise SkillAdmissionError("SKILL.md name is required")
    return name, frontmatter, text[end + 5 :]


def compile_native_skill(
    source: Path,
    *,
    source_id: str,
    ...
    name, frontmatter, body = _parse_skill(source)
    if registry.resolve(name) != canonical:
