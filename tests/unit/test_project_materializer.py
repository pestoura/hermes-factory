...
def test_materializes_stage_graph_on_product_board_and_authorizes_only_roots() -> None:
    ...
    assert {spec.revision for spec in adapter.specs} == {"d" * 64 + ".stage-contract-v21"}
...
def test_stage_contract_revision_versions_materialization_identity() -> None:
    ...
    expected = "d" * 64 + ".stage-contract-v21"
...
def test_candidate_branch_isolated_by_stage_contract_revision() -> None:
    ...
    assert a_discover.branch_name.endswith("-stage-contract-v21")
...
def test_stage_contract_requires_handoff_ready_completion() -> None:
    ...
    spec = _spec_by(adapter, "WP-A", "DISCOVER")
    assert spec.revision.endswith(".stage-contract-v21")
    assert spec.branch_name is not None
    assert spec.branch_name.endswith("-stage-contract-v21")
...
def test_stage_contract_explains_ancestor_repair_authority_for_cumulative_candidates() -> None:
    adapter = FakeKanbanAdapter()
    ProjectMaterializer(adapter).materialize(
        _model(), project_key="jarvas-cli", board="jarvas-cli",
        project_id="jarvas-cli", default_workdir="/srv/jarvas-cli",
    )
    review = _spec_by(adapter, "WP-A", "CODE_REVIEW")
    assert "repair_stage" in review.body
    assert "direct-parent candidate checkpoint" in review.body
    assert "earlier ancestor stage" in review.body
    assert "producer_stage" in review.body
...
def test_v19_evidence_only_stage_can_complete_without_repository_artifact() -> None:
    ...
    assert exact_sha.revision.endswith(".stage-contract-v21")
...
def test_v20_review_stage_declares_assigned_profile_is_independent_reviewer() -> None:
    base = _model()
    wp_a = replace(
        base.work_packages[0],
        stages=(
            "DISCOVER", "SPECIFY", "DESIGN", "TDD_RED", "IMPLEMENT",
            "CODE_REVIEW", "SECURITY_REVIEW", "ADVERSARIAL_REVIEW",
            "CI", "EXACT_SHA", "ACCEPT",
        ),
    )
    model = replace(base, work_packages=(wp_a, base.work_packages[1]))
    adapter = FakeKanbanAdapter()
    ProjectMaterializer(adapter).materialize(
        model, project_key="jarvas-cli", board="jarvas-cli",
        project_id="jarvas-cli", default_workdir="/srv/jarvas-cli",
    ...
    for stage in ("CODE_REVIEW", "SECURITY_REVIEW", "ADVERSARIAL_REVIEW"):
        body = _spec_by(adapter, "WP-A", stage).body
        assert "assigned Profile is already the independent reviewer" in body
        assert "independent_review_state=PASS" in body
        assert "Do not call kanban_request_review" in body
        assert "do not request a native Kanban review from yourself" in body


def test_v20_non_review_stage_does_not_claim_independent_reviewer_role() -> None:
    adapter = FakeKanbanAdapter()
    ProjectMaterializer(adapter).materialize(
        _model(), project_key="jarvas-cli", board="jarvas-cli",
        project_id="jarvas-cli", default_workdir="/srv/jarvas-cli",
    )
    body = _spec_by(adapter, "WP-A", "IMPLEMENT").body
    assert "assigned Profile is already the independent reviewer" not in body
...
def test_materialization_retires_superseded_generations_before_root_dispatch() -> None:
    ...
    revision = "d" * 64 + ".stage-contract-v21"
...
def test_v14_stage_contract_preserves_owned_artifact_namespace() -> None:
    ...
    assert discover.revision.endswith(".stage-contract-v21")
...
def test_v14_stage_contract_preserves_runtime_only_handoff_metadata() -> None:
    ...
    assert specify.revision.endswith(".stage-contract-v21")
...
def test_v18_stage_contract_requires_bounded_retry_safe_artifact_writes() -> None:
    ...
    assert design.revision.endswith(".stage-contract-v21")
